...
- **Dialog-Wiederholungstics:** Innerhalb derselben Figuren-Aussage (oder über wenige aufeinanderfolgende Sätze hinweg) nicht dieselbe Phrase doppelt verwenden. *„Wir haben ein Problem, wenn er dich gegen uns benutzt — haben wir ein Problem."* → einen der beiden Teile umformulieren. Bei knappen Charakteren (Komaru, Ghalrixtho) lieber drei kurze, **unterschiedliche** Stöße als eine längere Wiederholungs-Spirale.
- **Sinnesorgan und Wahrnehmung müssen zusammenpassen:** Wenn ein Charakter ein Geräusch wahrnimmt, sind nicht die Augen das beschreibende Organ, sondern das Gehör. Klingt trivial, rutscht aber leicht durch — besonders wenn ein Charakter-Merkmal (z.B. Ghalrixthos „rote Augen für die Dunkelheit") attraktiv klingt und in jeden Wahrnehmungskontext gepresst wird. Match Sinnesorgan zu Wahrnehmungsmodus.
...
- **Dialog-Wiederholungstics:** Wiederholt eine Figur dieselbe Phrase in kurzem Abstand? Umformulieren — bei knappen Charakteren lieber drei kurze, unterschiedliche Stöße.
- **Sinnesorgan ↔ Wahrnehmung:** Augen sehen, Ohren hören, Nase riecht. Wenn ein Charakter-Merkmal in den falschen Sinneskontext gepresst wird (z.B. „rote Augen registrierten ein Quietschen"), korrigieren.
...
3. **Doppelungen an Szenen-Grenzen straffen — auch in Charakter-Zuständen.** Beim getrennten Schreiben der Szenen rutschen oft Wiederholungen rein:
   - **Action/Setup-Doppelungen:** eine Bedrohung, die am Ende von Szene N etabliert wird, taucht zu Beginn von Szene N+1 nochmal als Setup auf.
   - **Charakter-Zustands-Doppelungen:** wenn Szene N damit endet, dass Ganden sich an die Konsole gesetzt hat und zu tippen begonnen hat, darf Szene N+1 nicht mit *„Ganden setzte sich an den Kommunikationsrechner"* öffnen — er ist schon da. Die Folge-Szene knüpft am Zustand an (*„An der Konsole tippte Ganden bereits..."*), etabliert ihn nicht neu.
   - Beim Mergen solche Wiederholungen entfernen, indem entweder die Endung der Vor-Szene oder die Eröffnung der Folge-Szene gekürzt wird.
4. **`---` als Section-Break sparsam einsetzen.** Trennstriche markieren echte Wechsel — von Schauplatz, Zeit oder Perspektive. Wenn eine Aktion direkt in die nächste übergeht (Ghalrixtho funkt eine Einschätzung ab und Ganden tippt im selben Moment am Comm), wird **kein** `---` gesetzt. Faustregel: Wenn der Leser nach dem Trennstrich noch in derselben Sekunde, am selben Ort, mit demselben emotionalen Atem fortsetzt, ist der Trennstrich falsch. Trennstriche sind kein Pacing-Werkzeug, sondern Wechsel-Marker.
5. Erstelle den Kapitel-Header:
...
6. Aktualisiere die Navigation im vorherigen Kapitel (füge Vorwärts-Link hinzu, falls nicht vorhanden).
7. Aktualisiere `chronicle/04-kapitel/README.md` – neuen Eintrag in die Inhaltsverzeichnis-Tabelle einfügen.
...
- Gibt es Widersprüche oder Wiederholungen zwischen Szenen — insbesondere Charakter-Zustands-Doppelungen?
- Werden Trennstriche nur an echten Wechseln gesetzt — nicht an direkten Aktions-Übergängen?
...
- **Sinnesorgan ↔ Wahrnehmung:** Wird ein Geräusch von Augen registriert oder ein Geruch von Ohren? Solche Mismatches korrigieren.
- **Dialog-Wiederholungstics:** Sagt eine Figur in kurzem Abstand zweimal dieselbe Phrase (*„wir haben ein Problem ... wir haben ein Problem"*)? Eine der beiden Stellen umformulieren.
- **Konsistenz-Grep auf häufige Schreibweisen-Drifts** — diese rutschen beim Mergen leicht durch, weil Szenen unabhängig geschrieben wurden:
  ```bash
  cd chronicle/04-kapitel
  grep -nE "TIE Fighter" kapitel-NN.md          # sollte 0 sein (immer mit Bindestrich)
  grep -nE "\bKomm-" kapitel-NN.md              # sollte 0 sein (Comm- mit zwei m)
  grep -nE "Komandobrücke" kapitel-NN.md        # sollte 0 sein (Kommandobrücke mit zwei m)
  grep -nE "dem selben" kapitel-NN.md           # sollte 0 sein (demselben, ein Wort)
  ```
  Diese Liste erweitert sich; bei neuen Eigennamen oder Sci-Fi-Begriffen der Kampagne sollte die kanonische Schreibweise einmal festgelegt und der Grep entsprechend ergänzt werden.
